(ns donut.frontend.routes
  ...
  ;; self-reference required for cljs consumers to get the clj-side
  ;; macros (defroutes, register-routes) via a plain :require —
  ;; without it, (dfr/defroutes ...) silently compiles as a call to an
  ;; undeclared var and never defines the routes var
  #?(:cljs (:require-macros [donut.frontend.routes])))
...
"
     ...
     (defn write-frontend-routes-hook
       ...
       [build-state route-var-name & [filename]]
       (if filename
         (write-defroutes route-var-name filename)
